...
class Report extends CI_Controller {
	...
	public function main() {
		...
		$this->pgToLoad = empty($this->pgToLoad) ? "report" : $this->pgToLoad;
		$disMsg = "";
	...
	public function displayReport() {
		if($this->uri->segment(2) == 'sss') { 
			$data['employees'] = $this->Contents->exetGetSSSReport($this->uri->segment(3));
			$data['payperiod'] = $this->Contents->exeGetPayperiod();
			$this->mainCont = $this->load->view('pages/sssreport', $data, TRUE);

		} elseif($this->uri->segment(2) == 'phic') { 
			$data['employees'] = $this->Contents->exetGetSSSReport($this->uri->segment(3));
			$data['payperiod'] = $this->Contents->exeGetPayperiod();
			$this->mainCont = $this->load->view('pages/philhealthreport', $data, TRUE);

		} elseif($this->uri->segment(2) == 'pagibig') { 
			$data['employees'] = $this->Contents->exetGetSSSReport($this->uri->segment(3));
			$data['payperiod'] = $this->Contents->exeGetPayperiod();
			$this->mainCont = $this->load->view('pages/pagibigreport', $data, TRUE);

		} elseif($this->uri->segment(2) == 'wtax') { 
			$data['employees'] = $this->Contents->exetGetSSSReport($this->uri->segment(3));
			$data['payperiod'] = $this->Contents->exeGetPayperiod();
			$this->mainCont = $this->load->view('pages/withholdingtaxreport', $data, TRUE);

		} else {
			$data['payperiod'] = $this->Contents->exeGetPayperiod();
			$this->mainCont = $this->load->view('pages/reportlist', $data, TRUE);
		}
	}
	...
	function pdf()
	{
			if($this->uri->segment(4) == 'print' ) { 
    			...
    			$data['employees'] = $this->Contents->exetGetSSSReport($this->uri->segment(3));
   				$data['payperiod'] = $this->Contents->exeGetPayperiod();
				   $this->load->view('pdfreport3',$data);
				}
